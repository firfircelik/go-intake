...
- `source/streaming` package: `FileTailSource` for real-time log monitoring
- `sink/cloud` package: `S3Sink` for AWS S3 uploads
- `sink/db` package: `SQLiteSink` for SQLite database exports
- `metrics` package: `PrometheusExporter` and `HealthCheck` for observability
- `window` package: `SlidingWindow` and `CountWindow` for streaming aggregation
- `enrich` package: `CacheLookup` and `StaticMapEnrich` for data enrichment
- `retry` package: `Policy` and `RetrySink` for error tolerance
